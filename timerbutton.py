...
Created = 2017-08-22
Description = Provides the TimerButton Tk widget, which contains a timer label,
    timer description, and button to start/stop the timer.
"""
# Tk imports
...
class TimerButton (tk.Frame):
    """TimerButton class
    Displays a grid Frame with a description label on the far left, count-up
    timer in the middle, and a button to start/stop the timer on the far right.
    """
    ...
    DEFAULT_SETTINGS = {
        ...
        'merge qualifications':{
            ...
            'delete short':True,
            # Allow pausing, entering description, then resuming
            'max adjacency distance':120.0,
    ...
    def _settings_version_update (self, oldsettings):
        ...
        if oldsettings['version'] \
                < TimerButton.OLDEST_CONVERTIBLE_SETTINGS_VERSION:
            raise VersionException(VersionException.TOO_OLD,
                oldsettings['version'],TimerButton.DEFAULT_SETTINGS['version'])
    ...
    def __init__ (self, parent, timerdata=None, timersettings=None,
            timertheme=None, *args, **options):
        """TimerButton constructor
        timerdata, timersettings, and timertheme are pre-populated versions of
        each of the defaults defined within this class, for initializing a
        timer button with non-default data/settings/theme.
        title is the very-short description of the timer (<15 chars).
        description is a longer description and will be truncated
        appropriately.
        """
        ...
        # Update the timerdata, timersettings, and timertheme parameters
        # (or set to defaults)
        self._data = helper.dictVersionUpdate(
            timerdata,
            self._data_version_update,
            self.DEFAULT_DATA
        )
        ...
        self._timer_label = tk.Label(
            self,text="00:00:00",font=self._timer_font
        )
        ...
        self._start_button = tk.Checkbutton(
            self,
            text=helper.PLAY_CHAR,
            command=self._toggle,
            font=self._start_font,
            variable=self._running,
            indicatoron=False
        )
    ...
    def _update_timer (self, restarttimer=True):
        ...
        Updates the label to the new time, as determined by the sum of the
        intervals in timer_data.intervals and the time between curr_start_time
        and time.time.
        If restarttimer is True, it fires an after with a 500ms delay to call
        this function.
        """
        ...
        # We've stored the most recent interval, so check if it should actually
        # exist. If the timer is now off, and we want to delete short
        # (i.e. mistake) intervals, and the most recent interval is short
        # enough, then delete it.
        deleteshort = self._settings['merge qualifications'] \
            ['delete short']
        shortdistance = self._settings['merge qualifications'] \
            ['max short distance']
        if (not restarttimer) and deleteshort \
                and self._curr_start_time != None \
                and end_time-self._curr_start_time <= shortdistance:
            del self._data['intervals'][-1]
        # Add up all the time from both the stored intervals and the
        # current interval
    ...
    def total_elapsed_time (self, end_time=0, unexportedonly=True):
        ...
        Add up all the time from both the stored intervals and the current
        interval.
        """
        sum_time = 0
    ...
    def _store_time (self,end_time):
        ...
        Either updates the current/last interval, or adds a new interval.
        The exact behavior is dependent on settings. It can be configured to
        replace overlapping intervals with unioned intervals, replace two
        nearby intervals with a single joined interval, or always store a new
        interval unless the previous start time exactly matches the current
        start time.
        """
        # If the timer is not started right now, there's nothing to store
        if self._curr_start_time == None:
            return
        # If there is no previous interval, skip all the later logic and just
        # add the new interval
        if len(self._data['intervals']) == 0:
            self._data['intervals'].append(
                [self._curr_start_time,end_time,False,""]
            )
            return
        # Next, check if the start times match exactly; if they do, skip the
        # later logic and just replace the latest interval
        # TODO: This is where we will check for very long intervals, which
        #       indicate the computer went to sleep.
        ...
        replace = False
        replaceOverlapping = self._settings['merge qualifications'] \
            ['overlapping intervals']
        replaceAdjacent = self._settings['merge qualifications'] \
            ['adjacent intervals']
        maxAdjacent = self._settings['merge qualifications'] \
            ['max adjacency distance']
        new_start_time = self._curr_start_time
        new_end_time = end_time
        old_start_time = self._data['intervals'][-1][0]
        old_end_time = self._data['intervals'][-1][1]

        # If the old interval contains the new start time, we should use the
        # old start time, but the new end time, erasing any future end time
        if replaceOverlapping and new_start_time > old_start_time \
                and new_start_time < old_end_time:
            # By definition, the overlapping and adjacent conditions cannot
            # happen at the same time, so if there is an overlap replacement,
            # do it now and exit
        ...
        # Finally, we've passed all the logic for joining intervals, we should
        # instead create new
        self._data['intervals'].append([new_start_time,new_end_time,False,""])

    def update_data (self):
        """update_data function
        Updates the labels from the data attribute. Used when the user updates
        the title/desc.
        """
    ...
    def update_theme (self):
        ...
        # Finally, turn off grid-propagation and force the width to be a
        # certain value. This way, the height will automatically expand
        # appropriately, but the width will remain a certain size for future
        # parent widgets. Make sure to grab the desired height first.
        self.update()
    ...
    def _truncate_description (self):
        ...
        Checks if the description widget should be truncated. If so, it grabs
        the maximum substring that fits within the label and tacks an ellipsis
        on the end.
        """
        ...
        if textwidth+padwidth <= labelwidth:
            ...
            return
        # Otherwise, decrementally check substrings of the label text
        # until it fits
    ...
    def _update_active_theme (self):
        ...
        Updates the colors to active or inactive, based on the state of
        the timer.
        """
        widgets = (
            (self,'widget'),
    ...
    def register_toggle_callback (self, callback):
        ...
        Registers a callback function which accepts one argument: this timer
        button widget. The callback function is called when the timer button
        toggles between active and inactive. This allows the main program to
        detect when the timer button is clicked, which in turn allows the main
        program to disable all other timers.
        """
    ...
    def register_labelclick_callback (self, callback):
        ...
        Registers a callback function which accepts one argument: this timer
        button widget. The callback function is called when any part of the
        TimerButton is clicked other than the start/pause button itself.
        This allows the main program to "select" the timer, which means it
        knows when to display the full data.
        """
        self._labelclick_callbacks.append(callback)
        self._title_label.bind(
            '<Button-1>',
            lambda e,self=self: callback(self)
        )
        self._desc_label.bind('<Button-1>', lambda e,self=self: callback(self))
        self._timer_label.bind(
            '<Button-1>', lambda e,self=self: callback(self)
        )
        self.bind('<Button-1>', lambda e,self=self: callback(self))

    def register_startqual_callback (self, callback):
        """Registers a callback which should accept 1 argument
        (the timerbutton) and return a boolean. This callback is called
        whenever the user attempts to start the timer. If it returns False,
        then the timer is not allowed to start."""

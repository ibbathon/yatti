...
class DataEditor (tk.Frame):
	...
	def __init__ (self, parent, conf, dataeditortheme=None, *args, **options):
		...
		The valid types are string, datetime, boolean, float, integer, interval, and table.
		However, you can extend this by mimicking the existing field[type].py modules.
		Example: Create a foo field type by creating a fieldfoo.py module with a FieldFoo class
		which inherits the FieldBase class.
		WARNINGS:
		1) You cannot recursively build tables, as it is unclear what that would look like.
		2) Calculated fields (e.g. interval) can only exist within tables and the 'sourceindexes'
		   key refers to the column indexes within the config, not the raw data.
		3) At the moment, calculated fields *must* be after their source columns. In other words,
		   you cannot have [datetime,interval[0,2],datetime]. This will hopefully be fixed in a
		   future reworking of the DataEditor.
		"""
		...
		# Load the basic types of fields; others will be dynamically loaded as needed
		...
		for i,field in enumerate(self._conf):
			...
			# Multi-row/column fields
				...
				for j,column in enumerate(field['columns']):
					...
					column['label'] = tk.Label(field['frame'],text=column['text'],
		...
		self._save_button = tk.Button(buttonframe,text="Save",font=self._save_button_font,
			command=self.save_data)
		self._save_button.pack(side='left')
		self._revert_button = tk.Button(buttonframe,text="Revert",font=self._save_button_font,
	...
	def _create_field (self, parent, conf, data=None):
		"""Checks to make sure the appropriate field module is loaded and then constructs one."""
	...
	def _load_basic_field_types (self):
		"""Loads the classes for strings, datetimes, booleans, floats, and integers."""
		for t in ('string','datetime','boolean','float','integer'):
			self._load_field_type(t)

	def _load_field_type (self, fieldtype):
		"""Loads the module 'field'+fieldtype and places the class 'Field'+Fieldtype in
		the dictionary of field types so that it can be used later."""
	...
	def _field_updated_callback (self):
		"""Callback for when a field is updated by the user. Changes any SAVED status fields
		to UNTOUCHED and then updates colors. This is so that we get rid of the green when
		the user starts modifying again."""
		for field in self._conf:
			if field['type'] != 'table':
				field['entry'].update_status(FieldBase.STATUS_UNTOUCHED,FieldBase.STATUS_SAVED)
			else:
				for column in field['columns']:
					for cell in column['rows']:
						cell.update_status(FieldBase.STATUS_UNTOUCHED,FieldBase.STATUS_SAVED)
		self.update_colors()
	...
	def _add_row (self, field, changestatus=True):
		...
		for i,column in enumerate(field['columns']):
			# If this is a raw-data column, let the field constructor use the default
	...
	def _regrid (self, field):
		...
		# Re-grid add button
		field['buttons'][-1].grid(column=0,row=numdelbuttons+1,columnspan=numcolumns+1)
	...
	def _update_single_field_color (self, entry):
		...
		if entry.status == entry.STATUS_UNTOUCHED:
			if issubclass(entry.__class__,tk.Entry) or issubclass(entry.__class__,tk.Text):
				key = 'untouched'
			else:
				section = 'base'
				key = 'widget'
	...
	def _build_table (self, field, data):
		"""Builds the table of the given field and fills it with the given data."""
		...
		for button in field['buttons']:
			...
			del button
		field['buttons'] = []
		# Now build it back up, by row and then column, so we can eventually have recursive
		# calculated fields. TODO: Actually implement said rec calc fields.
		numcols = field['numdatacolumns']
		for i in range(len(data)):
			for j, column in enumerate(field['columns']):
				# If this is a raw-data column, just feed the data into the constructor
				if 'dataindex' in column:
		...
		field['buttons'].append(button)
		button.grid(column=0,row=len(data)+1,columnspan=len(field['columns'])+1)
		# Finally, update themes
		field['status'] = FieldBase.STATUS_UNTOUCHED
		self.update_theme()

	def _update_single_field_data (self, conf, key, entry, column=None, row=None):
	...
	def load_data (self, data):
		"""Loads the provided data dict into the fields, building any tables in the process."""
		self._data = data
		for i,field in enumerate(self._conf):
	...
	def save_data (self):
		"""Uses the raw values in the entries to fill up the data dict, but only for enabled
		fields. Also handles rebuildng the list-of-lists in the data dict that corresponds
		to tables in the DataEditor."""
		...
		for i,field in enumerate(self._conf):
			...
			# For tables, we have two different types of enabled
				...
				if field['enabled']:
					numcols = field['numdatacolumns']
					numrows = len(field['columns'][0]['rows'])
					table = [[None for j in range(numcols)] for k in range(numrows)]
				# Otherwise, use the existing table
				else:
					table = self._data[field['key']]
				# Go through and update any enabled columns
				for j,column in enumerate(field['columns']):
					# If the column is raw data and either the column is enabled or the table
					# was rebuilt, populate this column
	...
	def check_all_field_statuses (self, status=None):
		"""Checks all fields for a given status. Returns True if a field has that status,
		False otherwise. If status is not provided, checks for UNSAVED or BADVALUE."""
		if status == None:
			# Easier to just recursively call this function twice than to have separate
			# logic for a None status
	...
	def update_theme (self):
		...
		helper.configThemeFromDict(self._revert_button,self._theme,'base','save button')
		for field in self._conf:
			...
			if field['type'] != 'table':
				...
				for column in field['columns']:
					helper.configThemeFromDict(column['label'],self._theme,'base','labels')
					for cell in column['rows']:
						helper.configThemeFromDict(cell,self._theme,'base','entries')
				for button in field['buttons']:
					helper.configThemeFromDict(button,self._theme,'base','buttons')
		# Finally update the unchanged/unsaved/errored/etc. font colors and enabled/disabled
		self.update_colors()
		self.update_states()

	def update_colors (self):
		"""Called whenever any field changes value, so that the font colors can be changed."""
		for field in self._conf:
			if field['type'] != 'table':
				self._update_single_field_color(field['entry'])
			else:
				for column in field['columns']:
					# For tables, we can also mark the frame background if we added/deleted a row
		...
		for field in self._conf:
			if field['type'] != 'table':
				if 'runningdisable' in field and field['runningdisable']:
					self._update_single_field_data(field,field['key'],field['entry'])
			else:
				# If the number of rows in the data differs from the number of rows in the
				# DataEditor table, the most likely cause is that the user just started a new
				# timer interval. In that case, we just want to add a new row.
				if len(self._data[field['key']]) == len(field['columns'][0]['rows']) + 1:
					self._add_row(field,changestatus=False)
					row = len(field['columns'][0]['rows'])-1
					for j,column in enumerate(field['columns']):
						cell = column['rows'][row]
						self._update_single_field_data(column,field['key'],cell,j,row)
				# If the row counts differ, but by more than 1, we need to rebuild
				# (potentially losing unsaved data)
				elif len(self._data[field['key']]) != len(field['columns'][0]['rows']):
					self._build_table(field,self._data[field['key']])
				# Finally, if the row counts match, just update the disabled columns
				else:
					for j,column in enumerate(field['columns']):
						if 'runningdisable' in column and column['runningdisable']:
							for k,cell in enumerate(column['rows']):
								self._update_single_field_data(column,field['key'],cell,j,k)
	...
	def disable_for_running (self):
		"""Disables all fields/columns which are marked for disabling while a timer is running."""
		for field in self._conf:
			if 'runningdisable' in field and field['runningdisable']:
				field['enabled'] = False
			if field['type'] == 'table':
	...
	def update_states (self):
		"""Enables/disables the entry fields according to the 'enabled' key in each field."""
		for field in self._conf:
			if field['type'] != 'table':
...
if __name__ == "__main__":
	...
	try:
		...
		config = [
			...
			{'type':'string','text':"Description",'key':'description','width':30},
			{'type':'string','text':"Source System",'key':'source system','width':20},
			{'type':'table','text':"Intervals",'key':'intervals','numdatacolumns':4,'columns':[
				{'type':'datetime','dataindex':0,'text':"Start",'width':19},
				{'type':'datetime','dataindex':1,'text':"End",'width':19},
				{'type':'interval','sourceindexes':[0,1],'text':"Length",'width':8},

...
Created = 2017-08-22
Description = Provides some helper functions and constants for use in my
    other programs.
"""
PLAY_CHAR=u'\u23F5'
PAUSE_CHAR=u'\u23F8'
...
DATE_FORMAT="%Y-%m-%d"
TIME_FORMAT="%H:%M:%S"
START_OF_TIME="1970-01-01"
DATE_TIME_SPACER=" "
DATETIME_FORMAT=DATE_FORMAT+DATE_TIME_SPACER+TIME_FORMAT
NUM_WEEKDAYS=7
WEEKDAYS=(
    ("S","U","Su","Sun","Sunday"),
    ("M","M","Mo","Mon","Monday"),
...
def dictFromDefaults (custom, defaults):
    ...
    Returns a dict with settings from custom and default settings from
    defaults. If custom is None or not a dict, then it returns a new dict.
    Otherwise, it modifies custom and returns a reference.
    """
    ...
    for k in defaults.keys():
        if k not in custom \
                or (type(defaults[k]) == type({}) \
                    and type(custom[k]) != type({}) \
            ):
    ...
    # Now we're done with copying everything on this level, so back up to
    # the previous level
    return custom

def configThemeFromDict (widget, theme, themetype, widgettype):
    """configThemeFromDict helper function
    If themetype exists in the theme dict and widgettype exists in
    theme[themetype], then configure the widget using the theme.
    If themetype is fonts, then use ** to expand the theme, as it does not
    work otherwise.
    """
...
def dictVersionUpdate (olddict, dictversionupdate, defaultdict):
    ...
    A wrapper which handles calling the version update function at the
    appropriate time.
    """
    # Make sure the dict is a dict
    if type(olddict) != type({}):
        olddict = {}
    # If the version attribute already exists in the olddict, then try to

...
Each instance provides a Tk widget with helper functions to assist the
DataEditor, including field-specific validation, an indicator of whether the
value has changed, a function to change the text without triggering
validation, and more.
Each inheriting class should implement its own _raw_to_viewable and
_viewable_to_raw.
...
class FieldBase (tk.Entry):
    """Base class for all data types. Tk widget with helper functions to
    assist the DataEditor.
    """
    ...
    def __init__ (self, parent, font, fieldconfig, data=None):
        ...
        data is either a single value or a list of other fields (for
        calculated).
        """
    ...
    def _default_value (self):
        """Returns the default value for this field type. Defined as a function
        so child classes can override default value without overriding init.
        """
        return ''

    def _validate_field (self, newval):
        """Validation function which checks if the newly-entered value is
        valid. If the new value is valid, set status to UNSAVED. Otherwise,
        set it to BADVALUE. Either way, always accept the new value.
        """
        ...
        # If we can successfully convert to raw data, then status should
        # be UNSAVED
    ...
    def set_raw (self, raw):
        """Sets the internal raw value and then converts it to viewable
        form and puts it in the field text.
        """
        if raw == None:
    ...
    def register_update_callback (self, cb):
        """Registers a given callback function to be called whenever the
        field text is updated to a valid value by the user.
        """

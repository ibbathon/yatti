...
class FieldInterval (fieldbase.FieldBase):
    ...
    def __init__ (self, parent, font, fieldconfig, data):
        """Initializes the field and calculates the raw from the provided
        other nodes. data must be a list of length 2 with elements being
        two other FieldBases, preferably (but not necessarily) FieldDatetimes.
    ...
    def calculate_raw_from_sources (self, preinit=False, automatic=False):
        """Calculates the raw interval length by subtracting the first source's
        raw from the second source's raw.
        """
    ...
    def _viewable_to_raw (self):
        """Converts the viewable time to an interval length using time and
        some manipulation. Also sets the raw value of the second source node
        to the sum of the interval length and the raw value of the first
        source node.
    ...
    def _raw_to_viewable (self):
        ...
        try:
            self.viewable = time.strftime(
                helper.TIME_FORMAT,time.gmtime(self.raw)
            )
            return True
        except:
            return False
    ...
    def test_class (fieldclass, testvalue=None):
        ...
        try:
            ...
            if testvalue != None:
                source[1] = fielddatetime.FieldDatetime(
                    root,font,None,source[0].raw+testvalue
                )
            else:
